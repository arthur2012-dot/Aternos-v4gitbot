 ...
 *
 * Every tick:
 *  1. Scan 3D voxel neighborhood (walls, gaps, soft, hard, water, sky)
 *  2. Enumerate ALL possible actions (walk, jump, dig, pillar, bridge, turn, stair, swim)
 *  3. Score each possibility
 *  4. Execute best action
 *  5. If stuck → sanitize corridor (dig 4 walls + ceiling + stair out)
 */
...
async function digHold(bot, block) {
  ...
  const timeout = !hasPick(bot) && stone ? 22000 : 14000;
  ...
  for (let t = 0; t < 4; t++) {
    try {
      ...
      if (!after || isAir(after)) return true;
    } catch {
      try {
        bot.stopDigging();
      } catch {}
...
function enumerateActions(scan, bot) {
  ...
  // SAFETY: lava → do nothing aggressive
  if (scan.inLava) {
  ...
  if (a0) {
    ...
    if (a0.softWall) acts.push({ type: 'dig_front', score: 120, block: a0.body });
    if (a0.wall && a0.body.kind === 'soft') acts.push({ type: 'dig_front', score: 130, block: a0.body });
    if (a0.wall && a0.body.kind === 'solid' && scan.trapped)
      acts.push({ type: 'dig_front', score: 150, block: a0.body });
    if (a0.gap) {
  ...
  // open directions as alternative walks
  for (const d of scan.dirScores) {
    if (d.airC >= 2) acts.push({ type: 'turn_walk', score: 60 + d.airC * 5, dx: d.dx, dz: d.dz });
    if (d.softC > 0 && d.score > 20)
      acts.push({ type: 'dig_dir', score: 70 + d.score, dx: d.dx, dz: d.dz, depth: 2 });
  }

  // always keep a baseline explore
  acts.push({ type: 'walk', score: 30 });
  acts.push({ type: 'turn_random', score: 15 });

  // sky / pillar for elevation progress
  ...
  switch (act.type) {
    ...
    case 'dig_front': {
      ...
      if (b && isSolid(b) && !isHard(b)) {
        console.log('[NAVTREE] dig_front', b.name);
    ...
    case 'dig_up': {
      ...
      if (b && isSolid(b) && !isHard(b)) {
        console.log('[NAVTREE] dig_up', b.name, 'y+' + y);
        return digHold(bot, b);
      }
      return false;
    }
...
export function startNavTree(agent) {
  ...
  const tick = async () => {
    ...
    // allow dig even if targetDigBlock set by us
    ...
    try {
      ...
      // force sanitize if still too long
      if (stillTicks >= 3 || scan.trapped) {
        const acts = enumerateActions(scan, bot);
        // boost sanitize / dig when still
        for (const a of acts) {
          if (a.type === 'sanitize' || a.type === 'dig_dir' || a.type === 'dig_up' || a.type === 'stair_out')
        ...
        if (best) {
          ...
          if (best.type === 'sanitize' || best.type === 'dig_dir') stillTicks = 0;
        }
        return;
      }

      // normal free movement decision tree
      const acts = enumerateActions(scan, bot);
      const best = acts[0];
      if (!best) return;

      // avoid repeating useless turn_random
  ...
  // physics-assisted micro decisions every tick while moving
  bot.on('physicsTick', () => {
    try {
      if (!bot.entity || busy) return;
  ...
  console.log('[NAVTREE] ON — pure code decision tree + area sanitation');
}
